...
## URI Validation
The main use that we expect you'll have for the `URI` class in Helpcore is validating and parsing Dash URIs. A `URI` instance exposes the address as a Helpcore `Address` object and the amount in satoshis, if present.
